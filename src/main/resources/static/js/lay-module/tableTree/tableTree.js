layui.define(['jquery'], function(exports) {
	let  MOD_NAME = 'treeTable', o = layui.jquery, tree = function() {};
	...
	tree.prototype.render = function(e){
		...
		if(e.url) {
			o.get(e.url, res=>{
				e.data = e.parseData(res);
				console.log("render table （url）", e);
				t.template(e);
			})
		} else {
			e.data =e.parseData(e.data);
			console.log("render table (data)", e);
			t.template(e);
		}
		return e;
	};
	...
	let model = new tree();
	console.log('init ' + MOD_NAME + ' model=>', model);
